...
def extract_tables_to_json(html_content, start_text, location_info, skip_tables=4):
    ...
    try:
        ...
        print(e)
        return []  # Return an empty list if table not found
...
def main(html_file_path, json_file_path, start_text):
    try:
        ...
        print(f"An error occurred while processing file '{html_file_path}': {e}")


if __name__ == "__main__":
    file_paths = glob.glob(r"D:\jjm\Maharashtra\Wardha\*\*\*\*.html")
    start_text = 'Village level functionaries contacts'

    for index, file_path in enumerate(file_paths):
        file_name = os.path.basename(file_path).replace('.html', '')
        json_file_path = rf'D:\jjm\extracted_json_data\Wardha\{index + 1}.json'
